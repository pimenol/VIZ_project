...
def make_demo_run(
    ...
    rise = 1.0 - np.exp(-s / max(peak_step / 2.0, 1.0))           
    decline = np.exp(-np.clip(s - peak_step, 0, None) / 30.0)    
    base_curve = 50.0 + 35.0 * rise * decline                     
...
def _make_demo_ss(
    ...
    base = np.full(n_residues, 2, dtype=np.uint8) 
    ...
    while pos < n_residues:
        ...
        label = np.uint8(rng.integers(0, 2)) 
        base[pos:end] = label
        pos = end
